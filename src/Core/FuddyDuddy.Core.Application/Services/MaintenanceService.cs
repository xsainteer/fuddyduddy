...
internal class MaintenanceService : IMaintenanceService
{
    ...
    {
        var summaries = await _summaryRepository.GetByStateAsync([NewsSummaryState.Validated, NewsSummaryState.Digested], dateStart: since, cancellationToken: cancellationToken);
